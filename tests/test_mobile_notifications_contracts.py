...
class MobileNotificationContractsTests(unittest.TestCase):
    ...
    def test_mobile_keeps_in_app_routing_without_native_push(self):
        ...
        settings = (ROOT / "mobile_app/screens/SettingsScreen.js").read_text(encoding="utf-8")
        center = (ROOT / "mobile_app/screens/NotificationsScreen.js").read_text(encoding="utf-8")
        self.assertNotIn("expo-notifications", mobile)
        self.assertNotIn("getExpoPushTokenAsync", mobile)
        self.assertNotIn("addNotificationResponseReceivedListener", navigator)
        self.assertNotIn("registerPushToken", navigator)
        self.assertNotIn("Notificaciones push", settings)
        self.assertNotIn("push_enabled", settings)
