...
class MobileProfileMediaTests(unittest.TestCase):
    ...
    def test_cached_profile_and_images_render_without_waiting_for_the_network(self):
        ...
        self.assertIn("useState(true)", profile)
        self.assertIn("useRef(false)", profile)
        self.assertIn("<ProfileSkeleton profile={profile} />", profile)
        skeleton = profile.split("function ProfileSkeleton", 1)[1]
        self.assertIn("<UserAvatar", skeleton)
        self.assertIn("profile?.avatar_url ?? profile?.foto_perfil", skeleton)
