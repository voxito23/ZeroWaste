            ...
            <tr class="border-b border-gray-50 dark:border-emerald-800/20 hover:bg-emerald-50/40 dark:hover:bg-white/[0.02] transition-colors stagger-row">
                <td class="p-3 pl-5">
                    <div class="flex items-center gap-3">
                        <div class="relative w-9 h-9 shrink-0 rounded-full border-2 {{$u->is_admin?'border-violet-400':'border-emerald-400'}} bg-emerald-100 dark:bg-emerald-900 flex items-center justify-center overflow-hidden">
                            <span class="font-black text-sm text-emerald-800 dark:text-emerald-100">{{mb_strtoupper(mb_substr(trim($u->nombre),0,1))}}</span>
                            @if($u->avatar_url)<img src="{{$u->avatar_url}}" alt="Avatar de {{$u->nombre}}" class="absolute inset-0 w-full h-full object-cover" loading="lazy" onerror="this.hidden=true;">@endif
                        </div>
                        <div>
