    ...
    <SafeAreaView className="flex-1 bg-slate-50" edges={['top', 'bottom']}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
          <ScrollView contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'on-drag'} showsVerticalScrollIndicator={false} bounces={false}>
            <View className="flex-1 px-6 pb-12">
              <View className="flex-row items-center py-3"><TouchableOpacity onPress={() => navigation.navigate('Login')} className="h-11 w-11 items-center justify-center rounded-full bg-white" accessibilityLabel="Volver al inicio de sesión"><ArrowLeft color="#0F172A" size={21} /></TouchableOpacity></View>
              <View className="flex-1 justify-center pb-8">
                ...
                <View className="relative mt-9">
                  <View pointerEvents="none" className="flex-row justify-between">{Array.from({ length: 6 }, (_, index) => <View key={index} className={`h-16 w-[14.5%] items-center justify-center rounded-2xl border-2 bg-white ${error ? 'border-red-300' : index === code.length ? 'border-emerald-500' : 'border-slate-200'}`}><Text className="text-2xl font-black text-slate-900">{code[index] || ''}</Text></View>)}</View>
                  <TextInput ref={inputRef} autoFocus value={code} onChangeText={updateCode} keyboardType="number-pad" maxLength={6} textContentType="oneTimeCode" autoComplete="one-time-code" caretHidden style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 64, opacity: 0 }} accessibilityLabel={`Código de verificación: ${code.length} de 6 dígitos capturados`} />
                </View>
