  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 40 }}>
        <RemoteImage uri={image.uri} fallbackSource={image.fallbackSource} className="w-full rounded-3xl bg-gray-50" style={{ height: 288 }} aspectRatio={null} resizeMode="contain" accessibilityLabel={`Imagen de ${reward?.nombre || 'la recompensa'}`} />
        <Text className="mt-6 text-3xl font-black">{reward?.nombre}</Text>
        <Text className="mt-3 text-base leading-6 text-gray-600" style={{ textAlign: 'justify' }}>{reward?.descripcion}</Text>
