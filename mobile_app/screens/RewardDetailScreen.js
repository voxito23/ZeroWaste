...
export default function RewardDetailScreen() {
  const navigation = useNavigation(); const reward = useRoute().params?.reward; const [submitting, setSubmitting] = useState(false); const requestLock = useRef(false);
  const redeem = () => Alert.alert('Confirmar canje', `Se utilizarán ${reward.costo_puntos} puntos.`, [{ text: 'Cancelar', style: 'cancel' }, { text: 'Canjear', onPress: async () => { if (requestLock.current) return; requestLock.current = true; setSubmitting(true); const idempotencyKey = `redeem-${Date.now()}-${Math.random().toString(36).slice(2)}`; try { await api.post('/impacto/canjes', { recompensa_id: reward.id, cantidad: 1, idempotency_key: idempotencyKey }); Alert.alert('Canje solicitado', 'Puedes consultar su estado en Mis canjes.', [{ text: 'Ver mis canjes', onPress: () => navigation.replace('MyRedemptions') }]); } catch (e) { Alert.alert('No se pudo canjear', e.userMessage); } finally { requestLock.current = false; setSubmitting(false); } } }]);
