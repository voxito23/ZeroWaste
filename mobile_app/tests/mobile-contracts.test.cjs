...
assert.match(comments, /enabled=\{Platform\.OS === 'ios'\}/);
...
assert.match(comments, /Easing\.inOut\(Easing\.cubic\)/);
assert.match(comments, /finished[\s\S]*requestAnimationFrame\(\(\) => inputRef\.current\?\.blur\(\)\)/);
