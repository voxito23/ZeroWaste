...
const collectionHistory = read('screens/MisRecoleccionesScreen.js');
const redemptions = read('screens/MyRedemptionsScreen.js');
...
const changePassword = read('screens/ChangePasswordScreen.js');
const helpSupport = read('screens/HelpSupportScreen.js');
...
assert.match(comments, /const \[keyboardVisible, setKeyboardVisible\] = useState\(false\)/);
...
assert.doesNotMatch(notifications, /expo-notifications|getExpoPushTokenAsync|expoPushToken|Firebase/);
assert.doesNotMatch(navigator, /expo-notifications|registerPushToken|addNotificationResponseReceivedListener|addPushTokenListener/);
assert.doesNotMatch(settings, /Notificaciones push|registerPushToken|unregisterPushToken|push_enabled/);
...
assert.match(notifications, /requester_avatar_url/);
assert.match(notificationsScreen, /Solicitó:/);
assert.match(notificationsScreen, /Destino:/);
assert.match(notificationsScreen, /Ver ruta/);
assert.match(notificationsScreen, /collection_created: Truck/);
...
assert.match(forum, /navigationBarTranslucent=\{false\}/);
assert.match(forum, /paddingBottom: Math\.max\(insets\.bottom, 24\)/);
assert.match(forum, /maxHeight: Math\.max\(180, screenHeight \* 0\.55\)/);
...
assert.doesNotMatch(settings, /El token push se desactivará/);
assert.match(settings, /void logout\(\)/);
...
assert.match(collectionHistory, /user\?\.rol !== 'recolector'/);
assert.match(collectionHistory, /\[user\?\.id\]/);
assert.match(collectionHistory, /setRecolecciones\(\[\]\)/);
assert.match(collectionHistory, /Recolecciones disponibles/);
assert.match(collectionHistory, /requestedAccountId !== activeAccountRef\.current/);
assert.match(collectionHistory, /loadedAccountId === accountId \? recolecciones : \[\]/);
...
assert.match(redemptions, /EN_PREPARACION:[\s\S]*label: 'En preparación'/);
assert.match(redemptions, /LISTA_PARA_ENTREGAR:[\s\S]*label: 'Lista para entregar'/);
...
assert.match(login, /visible=\{resetVisible\}[\s\S]*?<KeyboardAvoidingView[\s\S]*?<ScrollView/);
assert.match(login, /visible=\{resetVisible\}[\s\S]*keyboardDismissMode=/);
assert.match(emailVerification, /KeyboardAvoidingView[\s\S]*style=\{\{ flex: 1 \}\}/);
assert.match(emailVerification, /ScrollView contentContainerStyle=\{\{ flexGrow: 1 \}\}/);
...
assert.match(mapScreen, /const usingFallbackRef = useRef\(false\)/);
assert.match(mapScreen, /remountMap\(true\)/);
assert.match(mapScreen, /map: \{ flex: 1 \}/);
...
assert.match(createPost, /validatePickedForumImage/);
assert.match(createPost, /quality: 0\.45/);
assert.match(createPost, /navigationBarTranslucent=\{false\}/);
assert.match(createPost, /paddingBottom: Math\.max\(insets\.bottom, 24\)/);
...
assert.match(changePassword, /Contraseña y seguridad/);
assert.match(changePassword, /strengthLabel/);
assert.match(changePassword, /Mostrar \$\{label\.toLowerCase\(\)\}/);
assert.doesNotMatch(helpSupport, /FastAPI|JWT/);
...
assert.equal(appConfig.expo.android.googleServicesFile, undefined);
...
console.log('mobile-contracts.test.cjs: mapa, contenido, enlaces, comentarios y notificaciones internas correctos');
